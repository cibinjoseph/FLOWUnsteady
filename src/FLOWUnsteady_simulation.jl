...
function run_simulation(
    ...
    vpm.initialize_vpm!(pfield, dt, nsteps;
                        save_path=save_path, run_name=run_name*"_pfield",
                        verbose=verbose, verbose_nsteps=verbose_nsteps,
                        v_lvl=v_lvl,
                        create_savepath=create_savepath,
    ...
    # RUN
    for i in 0:nsteps
        breakflag = vpm.step_vpm!(i, pfield, dt, nsteps;
                              runtime_function=runtime_function,
                              static_particles_function=static_particles_function,
                              save_path=save_path,
                              create_savepath=create_savepath,
                              ...
                              save_time=false, vprintln=vprintln)
...
function initialize_simulation(
    ...
    return pfield, dt, runtime_function, static_particles_function, 
    line1, line2, run_id, file_verbose, vprintln, time_beg
end

function step_simulation(i, pfield, dt, nsteps::Int,
                              ...
                              run_name=nothing,
                              save_code="",
                              nsteps_save::Int=1, prompt=true,
                              verbose=true, verbose_nsteps=10, v_lvl=0,
                              save_time=false)
    ...
    breakflag = vpm.step_vpm!(i, pfield, dt, nsteps;
                              ...
                              save_path=save_path,
                              create_savepath=create_savepath,
                              run_name=run_name*"_pfield",
                              save_code=save_code,
                              nsteps_save=nsteps_save, prompt=prompt,
                              verbose=verbose, verbose_nsteps=verbose_nsteps, v_lvl=v_lvl,
                              save_time=save_time, vprintln=vprintln)

    return pfield, breakflag
end

function finalize_simulation(time_beg, line1, vprintln, run_id, v_lvl)
    # Finalize verbose
    vpm.finalize_verbose(time_beg, line1, vprintln, run_id, v_lvl)
end
